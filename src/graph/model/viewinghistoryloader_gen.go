...
// NewViewingHistoryLoader creates a new ViewingHistoryLoader given a fetch, wait, and maxBatch
func NewViewingHistoryLoader(config ViewingHistoryLoaderConfig) *ViewingHistoryLoader {
	return &ViewingHistoryLoader{
		fetch:    config.Fetch,
		wait:     config.Wait,
...
type ViewingHistoryLoader struct {
	...
	batch *viewingHistoryLoaderBatch
...
func (l *ViewingHistoryLoader) LoadThunk(key int) func() ([]*ViewingHistory, error) {
	...
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() ([]*ViewingHistory, error) {
			return it, nil
		}
	}
	if l.batch == nil {
		l.batch = &viewingHistoryLoaderBatch{done: make(chan struct{})}
	}
	batch := l.batch
...
func (l *ViewingHistoryLoader) LoadAll(keys []int) ([][]*ViewingHistory, []error) {
	...
	viewingHistorys := make([][]*ViewingHistory, len(keys))
	errors := make([]error, len(keys))
	for i, thunk := range results {
		viewingHistorys[i], errors[i] = thunk()
	}
	return viewingHistorys, errors
}
...
func (l *ViewingHistoryLoader) Prime(key int, value []*ViewingHistory) bool {
	...
	if _, found = l.cache[key]; !found {
		...
		cpy := make([]*ViewingHistory, len(value))
		copy(cpy, value)
		l.unsafeSet(key, cpy)
	}
	l.mu.Unlock()
...
func (b *viewingHistoryLoaderBatch) startTimer(l *ViewingHistoryLoader) {
	time.Sleep(l.wait)
	l.mu.Lock()
...
func (b *viewingHistoryLoaderBatch) end(l *ViewingHistoryLoader) {
