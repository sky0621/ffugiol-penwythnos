...
func testViewerToManyUserViewingHistories(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, viewerDBTypes, true, viewerColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Viewer struct: %s", err)
	}
	...
	check, err := a.UserViewingHistories().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := ViewerSlice{&a}
	if err = a.L.LoadUserViewingHistories(ctx, tx, false, (*[]*Viewer)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserViewingHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserViewingHistories = nil
	if err = a.L.LoadUserViewingHistories(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserViewingHistories); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testViewerToManyAddOpUserViewingHistories(t *testing.T) {
	var err error
	...
	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, viewerDBTypes, false, strmangle.SetComplement(viewerPrimaryKeyColumns, viewerColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserViewingHistories(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserViewingHistories[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserViewingHistories[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserViewingHistories().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
