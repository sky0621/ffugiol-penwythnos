...
type movieR struct {
	ViewingHistories ViewingHistorySlice
}
...
// ViewingHistories retrieves all the viewing_history's ViewingHistories with an executor.
func (o *Movie) ViewingHistories(mods ...qm.QueryMod) viewingHistoryQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"viewing_history\".\"movie_id\"=?", o.ID),
	)
...
// LoadViewingHistories allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (movieL) LoadViewingHistories(ctx context.Context, e boil.ContextExecutor, singular bool, maybeMovie interface{}, mods queries.Applicator) error {
	var slice []*Movie
	var object *Movie

	if singular {
		object = maybeMovie.(*Movie)
	} else {
		slice = *maybeMovie.(*[]*Movie)
	}
	...
	query := NewQuery(qm.From(`viewing_history`), qm.WhereIn(`viewing_history.movie_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ViewingHistories = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.MovieID {
				local.R.ViewingHistories = append(local.R.ViewingHistories, foreign)
				if foreign.R == nil {
					foreign.R = &viewingHistoryR{}
				}
...
// AddViewingHistories adds the given related objects to the existing relationships
// of the movie, optionally inserting them as new records.
// Appends related to o.R.ViewingHistories.
// Sets related.R.Movie appropriately.
func (o *Movie) AddViewingHistories(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*ViewingHistory) error {
	var err error
	...
	if o.R == nil {
		o.R = &movieR{
			ViewingHistories: related,
		}
	} else {
		o.R.ViewingHistories = append(o.R.ViewingHistories, related...)
	}
...
func FindMovie(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Movie, error) {
	movieObj := &Movie{}
	...
	query := fmt.Sprintf(
		"select %s from \"movie\" where \"id\"=$1", sel,
	)
...
func (o *Movie) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"movie\"", updateOnConflict, ret, update, conflict, insert)
...
func MovieExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"movie\" where \"id\"=$1 limit 1)"
