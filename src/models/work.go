...
var WorkWhere = struct {
	...
	ID:         whereHelperstring{field: "\"work\".\"id\""},
	Name:       whereHelperstring{field: "\"work\".\"name\""},
	Price:      whereHelpernull_Int64{field: "\"work\".\"price\""},
	CreateUser: whereHelpernull_String{field: "\"work\".\"create_user\""},
	CreatedAt:  whereHelpertime_Time{field: "\"work\".\"created_at\""},
	UpdateUser: whereHelpernull_String{field: "\"work\".\"update_user\""},
	UpdatedAt:  whereHelpernull_Time{field: "\"work\".\"updated_at\""},
	DeletedAt:  whereHelpernull_Time{field: "\"work\".\"deleted_at\""},
}
...
var (
	workAllColumns            = []string{"id", "name", "price", "create_user", "created_at", "update_user", "updated_at", "deleted_at"}
	workColumnsWithoutDefault = []string{"id", "name", "price", "updated_at", "deleted_at"}
	workColumnsWithDefault    = []string{"create_user", "created_at", "update_user"}
...
func (q workQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Work, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for work")
	}
...
func (q workQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count work rows")
	}
...
func (q workQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if work exists")
	}
...
func FindWork(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Work, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"work\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from work")
	}
...
func (o *Work) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no work provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"work\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"work\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into work")
	}
...
func (o *Work) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update work, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"work\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update work row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for work")
	}
...
func (q workQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for work")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for work")
	}
...
func (o WorkSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"work\" SET %s WHERE %s",
...
func (o *Work) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no work provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert work, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"work\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert work")
	}
...
func (o *Work) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from work")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for work")
	}
...
func (q workQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from work")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for work")
	}
...
func (o *WorkSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"work\".* FROM \"work\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, workPrimaryKeyColumns, len(*o))
...
func WorkExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"work\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if work exists")
	}
