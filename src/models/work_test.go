...
var (
	workDBTypes = map[string]string{`ID`: `character varying`, `Name`: `character varying`, `Price`: `bigint`, `CreateUser`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdateUser`: `character varying`, `UpdatedAt`: `timestamp without time zone`, `DeletedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
